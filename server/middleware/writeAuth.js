...
export function requireMemoryWriteAccess(req, res, next) {
  if (process.env.MEMORY_WRITE_AUTH_DISABLED === '1') {
    return next();
  }
  ...
  if (allowedEmails.length === 0 && !adminToken) {
    return res.status(403).json({
      error: 'Memory write access is not configured',
    });
  }

  return res.status(403).json({
    error: 'Memory write access denied or expired',
  });
}
